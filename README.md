...
7. Выполняет раннюю валидацию `src/` (синтаксис, локальные импорты, базовые NameError) и сохраняет отчет в `memory/source_validation_report.json`.
8. При ошибках валидации запускает pre-test цикл автоисправлений до генерации тестов.
9. Извлекает API из реального кода (`memory/api_description.json`) через `ast` (классы, функции, сигнатуры, публичные методы).
10. Генерирует тесты в `tests/` на основе извлеченного API и исходников.
11. Запускает тесты и пишет результаты в `tests/test_results.md`.
12. При ошибках запускает цикл автоисправлений через LLM (до 5 попыток), с возможностью править и `src/`, и `tests/`.
13. Финализирует проект (`memory/context.md`, `memory/bugs.md`) или создает `memory/blocked.md`.
...
- `agent/fixer.py` — LLM-исправление ошибок с классификацией причин (тесты/реализация).
...
Если после 5 попыток исправлений тесты не проходят, создается `memory/blocked.md`.
